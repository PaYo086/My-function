...
part.regre.plot <- function(v, y = 1, x = 2, line = TRUE, return.lm = FALSE){
  if (is.numeric(x) & is.numeric(y)){
    ...
    plot(resid(lm(y.formula, data = v[,-x])) ~ resid(lm(x.formula, data = v[,-y])), ylab = paste0("Residual of ", colnames(v)[y]), xlab = paste0("Residual of ", colnames(v)[x]))
    if (line){
    ...
    plot(resid(lm(res.v ~ ., data = v[, -order[2]])) ~ resid(lm(exp.v ~ ., data = v[, -order[1]])), ylab = paste0("Residual of ", y), xlab = paste0("Residual of ", x))
    if (line){
